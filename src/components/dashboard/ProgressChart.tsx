...
import HealthAppConnector from './HealthAppConnector';
...
export default function ProgressChart({ 
  ...
  const { t } = useLanguage();
  const [showHealthConnector, setShowHealthConnector] = useState(false);
  ...
  return (
    <>
      <div 
        className={cn(
          "glassy-card rounded-xl overflow-hidden card-shadow hover-scale", 
          isStepsChart && "cursor-pointer",
          className
        )}
        onClick={isStepsChart ? handleChartClick : undefined}
      >
              ...
              >
                ...
                {targetValue && (
                  <ReferenceLine 
                    ...
                    label={{
                      position: 'right',
                      value: `${t("target")}: ${targetValue}${label}`,
                      fill: '#FF6B6B',
                      fontSize: 10
                    }}
                  />
      ...
      <HealthAppConnector 
        isOpen={showHealthConnector} 
        onClose={() => setShowHealthConnector(false)}
      />
    </>
  );
}
