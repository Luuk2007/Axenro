...
import { TrendingUp, Dumbbell, ChevronRight, BarChart3, Target, Clock } from "lucide-react";
...
const WorkoutStatistics: React.FC<WorkoutStatisticsProps> = ({ workouts }) => {
  ...
  // If a group is selected, show exercise details for that group
  if (selectedGroup) {
      ...
      <div className="grid grid-cols-3 gap-3">
        <Card className="border-border/50">
          <CardContent className="p-4 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">{t("exercises")}</p>
          </CardContent>
        </Card>
        <Card className="border-border/50">
          <CardContent className="p-4 text-center">
            <p className="text-2xl font-bold">{sortedGroups.length}</p>
            <p className="text-xs text-muted-foreground mt-1">{t("Muscle Groups")}</p>
          </CardContent>
        </Card>
        <Card className="border-border/50">
          <CardContent className="p-4 text-center">
            <p className="text-2xl font-bold">{exerciseStats.reduce((sum, s) => sum + s.totalSets, 0)}</p>
            <p className="text-xs text-muted-foreground mt-1">{t("Total Sets")}</p>
          </CardContent>
        </Card>
      </div>
