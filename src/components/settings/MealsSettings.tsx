...
const MealsSettings: React.FC<Props> = ({ embedded }) => {
  ...
  const mealsContent = (
    <div className="space-y-3">
      <div className="space-y-2">
        ...
        <div className="space-y-2">
          {availableMeals.map((meal, index) => (
            <div key={meal.id || index} className="flex items-center justify-between p-2 border rounded-xl">
              ...
              {customMeals.find(cm => cm.id === meal.id) ? (
                <Button variant="ghost" size="sm" onClick={() => handleRemoveCustomMeal(meal.id)} className="text-destructive hover:text-destructive h-6 w-6 p-0">
                  <X className="h-3 w-3" />
                </Button>
              ) : (
                <Button variant="ghost" size="sm" onClick={() => handleRemoveDefaultMeal(meal.id)} className="text-destructive hover:text-destructive h-6 w-6 p-0">
                  <X className="h-3 w-3" />
                </Button>
              )}
            </div>
      ...
      {deletedMeals.length > 0 && (
        <div className="pt-2">
          <Button onClick={handleRestoreDefaultMeals} variant="outline" size="sm" className="rounded-xl">
      ...
      <div className="space-y-2">
        ...
        {showUpgradePrompt && (
          <div className="p-3 border border-orange-200 bg-orange-50 dark:bg-orange-950/30 dark:border-orange-800 rounded-xl">
            <div className="flex items-center gap-2 text-orange-800 dark:text-orange-300">
              ...
              <span className="text-sm font-medium">{t("Upgrade to add more custom meals")}</span>
            </div>
          </div>
        )}
        ...
        <div className="flex gap-2">
          ...
          />
          <Button onClick={handleAddCustomMeal} size="sm" className="h-9 rounded-xl" disabled={!canAddMoreMeals || loading}>
            {t("Add")}
          </Button>
        </div>
      </div>
  ...
  if (embedded) return mealsContent;
