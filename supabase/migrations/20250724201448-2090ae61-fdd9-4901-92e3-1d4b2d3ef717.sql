...
-- Regular users should not be able to update their subscription status directly
...
-- Users can only update their own email address, not subscription status
...
WITH CHECK (user_id = auth.uid() AND 
           OLD.subscribed = NEW.subscribed AND 
           OLD.subscription_tier = NEW.subscription_tier AND 
           OLD.subscription_end = NEW.subscription_end AND
